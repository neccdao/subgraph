...
export function runTests(): void {
  test("Position, entity is updated after IncreasePosition and UpdatePosition then DecreasePosition and ClosePosition events. Account and Action entity created and derived", () => {
    ...
    let mockClosePositionEvent = createClosePositionEvent(
      _key,
      size,
      collateral,
      averagePrice,
    ...
    addMetadata(mockClosePositionEvent);
    let closeTimestamp = BigInt.fromString("1997");
    mockClosePositionEvent.block.timestamp = closeTimestamp;

    let closePositionEvent = newMockEvent(mockClosePositionEvent);
    actionId =
      closePositionEvent.transaction.hash.toHexString() +
      "-" +
      closePositionEvent.logIndex.toString();

    handleClosePosition(closePositionEvent as ClosePosition);
    ...
    assert.fieldEquals("Action", actionId, "account", _account.toHexString());
    assert.fieldEquals("Action", actionId, "type", "ClosePosition");
